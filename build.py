      ...
      <form id="website-fit" class="brief-form quiz-form" action="https://formsubmit.co/{MAIL}" method="POST" novalidate>
        ...
        <fieldset class="quiz-step" data-step="3" hidden>
          ...
          <div class="form-grid">
            ...
            <label><span>Budget comfort zone</span><select name="budget" required><option value="" selected disabled>Choose a level</option><option>Keeping it lean</option><option>Ready for a solid business website</option><option>Planning a larger project</option><option>Not sure yet</option></select></label>
...
thanks = head(f"Thanks — {NAME}", "Thanks for getting in touch with Daejhonnel Denton.", "", "contact")
thanks += f"""
<section class="hero thanks"><div class="wrap">
  <p class="eyebrow">Message sent</p>
  <h1>Good. I’ve got it.</h1>
  <p class="lead">Your project brief is on its way to my inbox. I’ll read it and come back with the next sensible step.</p>
  <div class="herobtns"><a class="btn btn-p" href="index.html">Back home</a><a class="btn btn-g" href="work.html">See the work</a></div>
</div></section>
"""
thanks += foot("")
